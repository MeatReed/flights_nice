<script setup>
const { pending, data: flights } = useLazyFetch('http://192.168.1.87:5000/get_departure_flights_t1')
...
<template>
  <v-container>
    ...
    <v-row>
      <v-col>
        <v-btn block nuxt to="/departures/t1">
          T1
        </v-btn>
      </v-col>
      <v-col>
        <v-btn block nuxt to="/departures/t2">
          T2
        </v-btn>
      </v-col>
    </v-row>
    ...
    <v-row v-if="!pending && flights.departures">
      <v-col v-for="flight in flights.departures" cols="12">
        <v-card
          flat
          rounded="0"
          variant="outlined"
