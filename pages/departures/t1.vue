...
const { pending, data:flights } = await useLazyFetch(`${config.public.apiBase}/get_departure_flights_t1?page=${page}`)

async function fetchPrevious() {
  pending.value = true
  let { data:previousFlights } = await useLazyFetch(`${config.public.apiBase}/get_departure_flights_t1?page=${previousPage}`)
